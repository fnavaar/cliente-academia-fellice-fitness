...
- Autorizada a primeira leva de tasks de desbloqueio da Fase 2: F2-T001 a F2-T005 registram, separadamente, grade/duração/capacidade, responsável pela agenda, campos, consentimento, distribuição/encerramento e permissões. Todas permanecem bloqueadas até decisão humana, sem criar slots, agenda, contas, conectores ou publicação.
...
- **B3-03 concluída documentalmente:** em 24/09/2026, Karol confirmou “Karol confirma, libere as tasks”. A decisão preserva o vínculo da triagem no fluxo de agendamento, mantendo o `lead_submission_id` de origem quando a continuidade estiver disponível; casos sem vínculo permanecem visíveis como cobertura incompleta, sem atribuição por inferência. Nenhum código, banco, configuração, integração ou produto foi alterado.
- 2026-09-24 · Karol (Champion) · Task B3-03 concluída: decisão documental aprovada após conferência humana de Ricardo às 16:46; preservar o `lead_submission_id` de origem quando a continuidade estiver disponível e manter casos sem vínculo como cobertura incompleta, sem atribuição por inferência. Evidência em `03_documentos/decisoes-fase-3.md`; nenhum produto foi alterado.

## 2026-09-25

- 2026-09-25 · Ricardo Junior · DEBUG task `ab9ce40f-304e-4ef6-929f-1efa7f7a0a87`: login do `/funil` devolvia mensagem combinada; logs confirmaram HTTP 400 `Failed to authenticate` antes da validação de papel. v0.0.81 separa falha de autenticação de papel sem acesso; QA passou e preview confirmou a mensagem com conta fictícia. Causa específica das contas reais permanece indeterminada (conta ausente ou senha incorreta); contas/permissões não foram alteradas, teste autenticado segue bloqueado. Nota: `06_notas/debug/debug-2026-09-25-spec-3-001-login-funil.md`.
- 2026-09-25 · Ricardo Junior · Task `ab9ce40f-304e-4ef6-929f-1efa7f7a0a87`: após provisionamento informado pelo responsável, logs registraram três criações de registros de autenticação com HTTP 200 e backend compartilhado; não há logs posteriores de autenticação ou leitura do funil. Preview v0.0.81 segue ativo, produção não publicada; SPEC-3-001 em `aguardando_teste_humano` até validação autenticada das contagens e da cobertura.
- 2026-09-25 · Ricardo Junior · Teste humano de `ab9ce40f-304e-4ef6-929f-1efa7f7a0a87`: conferência da janela isolada 2026-09-23 aprovada pelo responsável (“os valores batem com as fixtures da SPEC-3-001”). Log autenticado `GET /backend/v1/funnel?from=2026-09-23&to=2026-09-23` retornou HTTP 200 às 19:38:30 UTC. CA-3.01..04 têm evidência de teste humano para essa janela; recálculo determinístico e rollback sem perda permanecem pendentes. Task continua aberta; produção não publicada.
- 2026-09-25 · Ricardo Junior · Task `ab9ce40f-304e-4ef6-929f-1efa7f7a0a87` concluída no preview: consolidação somente leitura validada. Teste humano: fixtures da janela 23/09 conferidas; recálculo determinístico e rollback/reativação aprovados. Evidência técnica: QA completo v0.0.81; migrações 0050–0051 aplicadas; logs autenticados de consultas e controles HTTP 200; harness isolado do hook simulando falha de leitura de `lead_submissions` confirmou resposta parcial, taxa nula e identificação da fonte com erro, sem escrita. CA-3.01..05 validados para a tarefa de implementação. Produção não publicada; artefatos de captura/export sanitizado e aceite formal da SPEC devem ser conferidos na task separada `4217198c-8077-4769-b2c0-7b7482136553`.
